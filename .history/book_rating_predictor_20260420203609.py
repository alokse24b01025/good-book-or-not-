...
# ─────────────────────────────────────────────
# STEP 6: SMART PREDICTION (ML + RULE BASED)
# ─────────────────────────────────────────────
print("\n[6/6] Prediction...")
...
sample = pd.DataFrame([[rating, reviews, pages]], columns=features)

# ---------------- ML PREDICTION ----------------
if best_name == "Logistic Regression":
    sample_scaled = scaler.transform(sample)
    ml_pred = best_model.predict(sample_scaled)[0]
    prob = best_model.predict_proba(sample_scaled)[0]
else:
    ml_pred = best_model.predict(sample)[0]
...
# Rule 1: High rating
conditions.append(rating >= 4.2)

# Rule 2: Enough reviews
conditions.append(reviews >= 1000)

# Rule 3: Reasonable pages
conditions.append(100 <= pages <= 800)

# Count how many conditions are true
true_count = sum(conditions)

# Rule decision
rule_pred = 1 if true_count >= 2 else 0


# ---------------- FINAL DECISION ----------------
# Combine ML + Rules
if ml_pred == 1 and rule_pred == 1:
    final_pred = 1
elif ml_pred == 1 or rule_pred == 1:
    final_pred = 1
else:
    final_pred = 0
...
# ---------------- OUTPUT ----------------
print("\n--- RESULT ---")
print(f"ML Prediction        : {'GOOD' if ml_pred == 1 else 'NOT GOOD'}")
print(f"Rule-based Decision  : {'GOOD' if rule_pred == 1 else 'NOT GOOD'}")
print(f"Conditions matched   : {true_count}/3")

print("\nFINAL RESULT:",
      "GOOD BOOK ✅" if final_pred == 1 else "NOT GOOD ❌")
